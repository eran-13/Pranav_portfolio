...
const PinterestSection = () => {
  ...
  const [pins, setPins] = useState(fallbackPins);
  useDragToScroll(scrollContainerRef);

  // Load from database, fallback to hardcoded
  const loadPins = async () => {
    try {
      const mediaFiles = await getCombinedMedia('pinterest', 'image');
      
      if (mediaFiles.length > 0) {
        const dbPins = mediaFiles.map((file, index) => ({
          id: index + 1,
          image: file.file_url,
        }));
        setPins(dbPins);
  ...
  return (
    <section id="pinterest" className="relative py-24 px-4 overflow-hidden">
      ...
      <div className="container mx-auto max-w-7xl" ref={ref}>
        ...
        >
          ...
          <h2 className="font-display text-4xl md:text-5xl font-bold">
            Pinterest <span className="neon-text">Pins</span>
          </h2>
        </motion.div>
